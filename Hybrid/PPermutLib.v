...
Inductive PPermut_LF: list Context_LF -> list Context_LF -> Prop :=
| PPermut_LF_nil: PPermut_LF nil nil
| PPermut_LF_skip: forall G G' w Gamma Gamma',
    Gamma *=* Gamma' ->
    PPermut_LF G G' ->
    PPermut_LF ((w, Gamma)::G) ((w,Gamma')::G')
| PPermut_LF_swap: forall G w w' Gamma0 Gamma0' Gamma1 Gamma1',
    Gamma0 *=* Gamma1 ->
    Gamma0' *=* Gamma1' ->
    PPermut_LF ((w, Gamma0) :: (w', Gamma0') :: G)
                ((w', Gamma1') :: (w, Gamma1) :: G)
...
Notation "G ~=~ G'" := (PPermut_LF G G') (at level 70) : permut_scope.
...
induction H; eauto with ppermut_rew.
  apply PPermut_LF_trans with (G':=(w, Gamma')::G); eauto with ppermut_rew;
...
Add Setoid (list Context_LF) PPermut_LF PPermut_LF'oid as PPermut_LFoid.

Add Morphism (@cons Context_LF) : PPermut_LF_cons.
...
Add Morphism (@append Context_LF) : PPermut_LF_app.
...
Proof.
  ...
  destruct l1; destruct l3; rew_app in *; inversion H6; inversion H7; subst;
    auto.
      constructor; auto; transitivity a'; auto; transitivity a; auto;
       ...
       symmetry in Hp; apply PPermut_LF_nil_impl in Hp; destruct l1; rew_app in *;
         discriminate.
  ...
  destruct l1; destruct l3; rew_app in *; inversion H6; inversion H7; subst;
    auto.
       ...
       apply PPermut_LF_nil_impl in Hp; destruct l3; rew_app in *; discriminate.
       rewrite <- Hp; PPermut_LF_simpl; transitivity ((w,x)::(w0,y)::nil);
         PPermut_LF_simpl; constructor; auto; constructor; auto; transitivity a';
         auto; symmetry; auto.
       transitivity ((w',y')::(w,x')::l1); auto; PPermut_LF_simpl; rew_app;
       replace l1 with (l1 ++ nil) by (rew_app; auto);
       apply IH with (w:=w0) (a0:=a) (a'0:=a'); auto;
         rew_app; auto.
  ...
  destruct l3; rew_app in *; inversion H7; subst.
    symmetry in Hp; apply PPermut_LF_nil_impl in Hp; subst; inversion H6; subst;
      ...
      discriminate.
    transitivity ((w', y') ::l1 ++ p0 :: l2); PPermut_LF_simpl; rew_app.
  ...
  inversion H7; subst.
    ...
    transitivity ((w', y'):: p0 :: l2).
      constructor; auto.
      PPermut_LF_simpl. clear H6 H7. rew_app; rewrite Hp; PPermut_LF_simpl;
    ...
    transitivity ((w', y') :: (w, x') :: l1 ++ p0 :: l2).
      ...
      PPermut_LF_simpl. clear H7 H6. rew_app.
    apply IH with (w:=w0) (a0:=a) (a'0:=a'); rew_app; auto.
  (* trans *)
  intros. destruct (PPermut_LF_Mem l l' w a) as (a'', (H6a, H6b)); auto.
...
apply PPermut_LF_last_rev_simpl with (a:=(w, Gamma)); auto.
Qed.
...
intros.
apply PPermut_LF_Mem with (w:=w) (X:=Gamma) in H.
...
assert ((w, Gamma) ::G  ~=~ (G' & (w', Gamma'))) by
  (transitivity (G & (w,Gamma)); auto; PPermut_LF_simpl).
