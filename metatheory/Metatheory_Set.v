(***************************************************************************
* A small library of non-constructive finite sets                          *
...
(** This file contains an interface for finite sets. For simplicity and
    efficiency, this interface purposely does not rely on type-classes.
    Moreover, the interface aims at including only the lemmas about finite
    sets that are actually useful for metatheory reasoning, and not all
    the possible results about finite sets.

    We show how the interface presented can be given a concrete   
    implementation, by building on top of the (possibly-infinite) sets
    (from [tlc/LibSet.v]) and constructing dependent pairs in order to
    attach a finiteness proof to the sets. *)
...
Lemma notin_empty : forall x,
  x \notin \{}.
Proof. intros_all. false* in_empty. Qed.
