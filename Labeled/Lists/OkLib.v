...
Lemma ok_Gamma_used_permut:
forall G U U',
  U *=* U' -> ok_Gamma G U -> ok_Gamma G U'.
induction G; intros.
...
apply ok_Gamma_used_permut with (U' := (u::v0::U)) in H0;
...
apply ok_Gamma_used_permut with (U:=v2::v0::U); [permut_simpl | auto].
intros; auto.
Qed.
...
contradiction.
apply ok_Gamma_used_permut with (U:=x::v0::U). permut_simpl.
apply IHG; auto.
...
apply ok_Gamma_extend_fresh; auto.
...
apply ok_Gamma_permut with (G2:=(w, (x,A))::hd++tl) in H2.
...
simpl in *; destruct H0; apply ok_Gamma_weakening_used with (u:=x); auto.
Qed.
...
apply ok_Gamma_permut with (G2:= (w', (x, b))::hd ++ tl) in H0;
...
Lemma ok_Gamma_rename:
forall G U w w',
  ok_Gamma G U ->
  ok_Gamma (rename_context_L w w' G) U.
