...
induction H; eauto with ppermut_rew.
  apply PPermut_trans with (G':=A'::G); eauto with ppermut_rew;
  apply PPermut_skip;
  [ apply permut_sym |
...
Add Setoid (list ctx_LF) PPermut PPermut'oid as PPermutoid.

Add Morphism (@cons ctx_LF) : PPermut_cons.
...
Add Morphism (@append ctx_LF) : PPermut_app.
...
Proof.
  ...
  destruct l1; destruct l3; rew_app in *; inversion H6; inversion H7; subst.
      constructor; auto; transitivity a'; auto; transitivity a; auto;
       ...
       symmetry in Hp; apply PPermut_nil_impl in Hp; destruct l4; rew_app in *;
         discriminate.
       clear H2 H3 H6 H7.
  ...
  destruct l1; destruct l3; rew_app in *; inversion H6; inversion H7; subst;
    auto.
       ...
       apply PPermut_nil_impl in Hp; destruct l3; rew_app in *; discriminate.
       rewrite <- Hp; PPermut_simpl; rew_app; apply PPermut_swap;
       [ transitivity a' | ]; auto; symmetry; auto;
       clear H7 H3 H2 H6;
       transitivity (l2::l3::l6); auto; PPermut_simpl; rew_app;
       replace l6 with (l6 ++ nil) by (rew_app; auto);
       apply IH with (a0:=a) (a'0:=a'); auto;
         rew_app; auto.
    transitivity (l1::l0::l6); [PPermut_simpl|];
  ...
  destruct l6; rew_app in *; inversion H7; subst.
    symmetry in Hp; apply PPermut_nil_impl in Hp; subst; inversion H6; subst;
      ...
      discriminate.
    transitivity (l4 ::l1 ++ l2 :: l5); PPermut_simpl; rew_app.
  ...
  inversion H7; subst.
    ...
    transitivity (l1:: l2 :: l5).
      constructor; auto.
      PPermut_simpl. rew_app; rewrite Hp; PPermut_simpl;
    ...
    transitivity (l6 :: l3 :: l8 ++ l2 :: l5).
      constructor; auto; constructor; auto.
      PPermut_simpl. rew_app.
    apply IH with (a0:=a) (a'0:=a'); rew_app; auto.
  (* trans *)
  intros. destruct (PPermut_Mem l l' a) as (a'', (H6a, H6b)); auto.
...
apply PPermut_last_rev_simpl with (a:=Gamma); auto.
Qed.
...
intros.
apply PPermut_Mem with (X:=Gamma) in H.
...
assert (Gamma ::G  ~=~ (G' & Gamma')) by
  (transitivity (G & Gamma); auto; PPermut_simpl).
